...
uint32_t RCC_GetPCLL1Value(void){

	uint32_t pclk1,SystemClk;
	uint8_t clksrc,temp,ahb1p,apb1;
...
void I2C_Init(I2C_Handle_t *pI2CHandle){
    ...
    tempreg = 0;
    tempreg |= RCC_GetPCLL1Value()/1000000U ;
    ...
    if(pI2CHandle->I2C_Config.I2C_SCLSpeed <= I2C_SCL_SPEED_SM){
    	...
    	ccr_value = (RCC_GetPCLL1Value()/ (2*pI2CHandle->I2C_Config.I2C_SCLSpeed));
    ...
    else
    {
    	...
    	{
    		ccr_value = (RCC_GetPCLL1Value() / (3 * pI2CHandle->I2C_Config.I2C_SCLSpeed));

    	}
    	else{
    		ccr_value =(RCC_GetPCLL1Value() / (25*pI2CHandle->I2C_Config.I2C_SCLSpeed));
    ...
    if(pI2CHandle->I2C_Config.I2C_SCLSpeed <= I2C_SCL_SPEED_SM){
       	...
       	tempreg=(RCC_GetPCLL1Value()/1000000U)+1;

       }
       else
       {
          	tempreg=(RCC_GetPCLL1Value()*300/1000000000U)+1;
...
//	Until SB is cleared SCL will be stretched (pulled to LOW)
	 ...
	 //3.Send the address of the slave  with r/w bit set to w(0) (total 8 bits)
       ...
       //5. clear the addr flag accroding to its software sequence
...
// 3.Handle for the interrupt generated by BTF (BYte transfer finished) event
	if(temp1 && temp3){
...
//	 note : Stop detection flag is applicable only slave mode. for master this
	if(temp1 && temp3){
			//STOPF flag is set
